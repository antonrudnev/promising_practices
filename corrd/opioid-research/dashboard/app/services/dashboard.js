...
function (angular, $, kbn, _, config, moment, Modernizr) {
    ...
    module.service('dashboard', function ($routeParams, $http, $rootScope, $injector, $location,
        ...
        var route = function () {
            ...
            if (!(_.isUndefined($routeParams.category))) {
                var _type = $routeParams.category;

                switch (_type) {
                    case ('prevention_and_education'):
                        self.file_load('prevention_and_education.json');
                        break;
                    case ('safe_prescribing'):
                        self.file_load('safe_prescribing.json');
                        break;
                    case ('overdose_prevention_and_response'):
                        self.file_load('overdose_prevention_and_response.json');
                        break;
                    case ('treatment_delivery_models'):
                        self.file_load('treatment_delivery_models.json');
                        break;
                    case('legal_reforms_and_policies'):
                        self.file_load('legal_reforms_and_policies.json');
                        break;
                    case('medication_assisted_treatment'):
                        self.file_load('medication_assisted_treatment.json');
                        break;
                    case('psychosocial_therapy'):
                        self.file_load('psychosocial_therapy.json');
                        break;
                    default:
