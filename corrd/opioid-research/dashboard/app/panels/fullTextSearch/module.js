...
define([
  ...
  function (angular, app, _, kbn, moment) {
    ...
    // custom filter for result tables
    module.filter('condensedTable', function () {
      return function (result) {

        var cleanedResult = {
          "This intervention is an example of:": result.intervention_name ? result.intervention_name : "No data",
          "Implementers:": result.implementers ? result.implementers : "No data" ,
          "Goal of this intervention:": result.intervention_goal ? result.intervention_goal : "No data",
          "Who will benefit:": result.population ? result.population : "No data",
          "Components of the program:": result.program_components ? result.program_components : "No data",
          "What was the result?": result.program_success ? result.program_success : "No data"
        };

        return cleanedResult;
      }
    });
    module.filter('expandedTable1', function () {
      return function (result) {
        var cleanedResult = {
          "City:": result.city ? result.city : "No data",
          "Geography:": result.geography ? result.geography : "No data"
        };

        return cleanedResult;
      }
    });
    module.filter('expandedTable2', function () {
      return function (result) {
        var cleanedResult = {
          "Summary": result.public_summary ? result.public_summary : "No data"
        };
